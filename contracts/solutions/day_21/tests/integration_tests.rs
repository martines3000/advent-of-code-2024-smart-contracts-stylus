...
// Test Day 21 Part 1 (2024)
#[e2e::test]
async fn day_21_1(account: Account) -> Result<()> {
    ...
    assert!(result == 197560);
    Ok(())
}

// Test Day 21 Part 2 (2024)
#[e2e::test]
async fn day_21_2(account: Account) -> Result<()> {
    let contract_addr = account.as_deployer().deploy().await?.address()?;
    let instance = Solution::new(contract_addr, &account.wallet);
