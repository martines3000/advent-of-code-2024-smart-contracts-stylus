...
// Test Day 9 Part 1 (2024)
#[e2e::test]
async fn day_9_1(account: Account) -> Result<()> {
    ...
    assert!(result == 6216544403458);
    Ok(())
}

// Test Day 9 Part 2 (2024)
#[e2e::test]
async fn day_9_2(account: Account) -> Result<()> {
    let contract_addr = account.as_deployer().deploy().await?.address()?;
    let instance = Solution::new(contract_addr, &account.wallet);
